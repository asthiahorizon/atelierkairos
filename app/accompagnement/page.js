...
import { Leaf, HandHeart, Brain, Shield, BrainCircuit, ArrowUpRight, Wind, Flower2, Sparkles, Compass, ShieldCheck } from 'lucide-react';
  ...
  return (
    <main className="min-h-screen bg-[#f5f4f8]">
      ...
      <section className="py-16 md:py-24">
        <div className="container mx-auto px-6 grid lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-7 space-y-5 text-[#3730a3]/75 leading-relaxed text-lg">
            <p>Mon accompagnement s&apos;appuie sur une approche psycho-corporelle — <span className="text-[#3730a3] font-medium">point d&apos;entrée privilégié, qui reste fluide et intégrative.</span></p>
            <p>Elle prend en compte toutes les dimensions de l&apos;humain — psychologique, émotionnelle, physique et spirituelle — en favorisant la régulation du système nerveux, la reconnexion au corps, aux émotions, à l&apos;environnement et à la conscience, <span className="italic text-[#4f46e5]">afin de soutenir l&apos;expression de l&apos;Être.</span></p>
          </div>
        </div>
      </section>
            ...
            {METHODS.map((m) => (
              <article key={m.name} className="group glass rounded-3xl p-8 hover:glass-strong hover:-translate-y-1 transition-all duration-500">
                <div className="flex items-start justify-between mb-10">
                  <div className="w-14 h-14 rounded-2xl bg-[#4f46e5]/12 flex items-center justify-center group-hover:bg-[#4f46e5]/22 transition-colors">
                    <m.icon className="w-6 h-6 text-[#4f46e5]" strokeWidth={1.4} />
                  </div>
                  <span className="font-serif text-5xl text-[#4f46e5]/25 italic group-hover:text-[#4f46e5]/45 transition-colors">{m.short}</span>
                </div>
                <h3 className="font-serif text-2xl text-[#3730a3] mb-4 leading-tight">{m.name}</h3>
                <p className="text-[#3730a3]/70 leading-relaxed text-[15px]">{m.text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 md:py-28 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6">
          <SectionTitle kicker="Les orientations" title="Orientations" italic="d'accompagnement." subtitle="Deux axes de travail distincts, conçus pour répondre à des besoins spécifiques tout en s'appuyant sur la même approche psycho-corporelle." />
                ...
                <div className="p-8 md:p-10 lg:p-12">
                  <div className="flex items-start gap-6 mb-8">
                    <div className="w-16 h-16 rounded-2xl glass-indigo flex items-center justify-center shrink-0">
                      <o.icon className="w-7 h-7 text-[#4f46e5]" strokeWidth={1.3} />
                    </div>
                    <div className="flex-1">
                      <h3 className="font-serif text-2xl md:text-3xl lg:text-[34px] text-[#3730a3] leading-tight tracking-tight">{o.title}</h3>
                      <p className="mt-3 font-serif italic text-lg md:text-xl text-[#4f46e5] leading-snug">{o.lead}</p>
                    </div>
                  </div>
                  <div className="space-y-4 text-[#3730a3]/75 leading-relaxed">
                  ...
                  <div className="mt-7 pt-6 border-t border-[#3730a3]/10">
                    <p className="text-[10px] uppercase tracking-[0.25em] text-[#3730a3]/55 mb-4">Indications possibles</p>
                    <div className="flex flex-wrap gap-2">
                      {o.indications.map((t) => (
                        <span key={t} className="text-xs md:text-sm px-3.5 py-1.5 rounded-full glass-indigo text-[#3730a3]">{t}</span>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            {INDICATIONS.map((ind, i) => (
              <div key={ind.title} className="group glass rounded-3xl p-8 hover:glass-strong transition-all duration-500">
                <div className="flex items-start justify-between mb-6">
                  <div className="w-12 h-12 rounded-2xl bg-[#4f46e5]/12 flex items-center justify-center group-hover:bg-[#4f46e5]/22 transition-colors">
                    <ind.icon className="w-5 h-5 text-[#4f46e5]" strokeWidth={1.4} />
                  </div>
                  <span className="font-serif text-sm text-[#3730a3]/40">0{i + 1}</span>
                </div>
                <h3 className="font-serif text-2xl text-[#3730a3] mb-3 leading-tight">{ind.title}</h3>
                <p className="text-[#3730a3]/70 leading-relaxed text-[15px]">{ind.text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-3xl mx-auto text-center mb-14">
            <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl leading-[1.04] tracking-tight text-[#3730a3]">
              Une <span className="italic font-light text-[#4f46e5]">tarification</span> juste et accessible.
            </h2>
          </div>
          <div className="grid md:grid-cols-2 gap-5 max-w-5xl mx-auto">
            <div className="glass-strong rounded-3xl p-10 md:p-12">
              <p className="text-xs uppercase tracking-widest text-[#3730a3]/50 mb-5">Tarif standard</p>
              <div className="flex items-baseline gap-2 mb-2"><span className="font-serif text-7xl md:text-8xl text-[#3730a3] font-light tracking-tight">130</span><span className="text-[#3730a3]/60 text-xl">CHF</span></div>
              <p className="text-[#3730a3]/60">par séance d&apos;1 heure</p>
              <div className="my-8 h-px bg-[#3730a3]/10" />
              <ul className="space-y-3 text-[#3730a3]/75 text-sm">
                <li className="flex gap-3"><Leaf className="w-4 h-4 text-[#4f46e5] shrink-0 mt-0.5" strokeWidth={1.5} /> Séance individuelle de 60 minutes</li>
                <li className="flex gap-3"><Leaf className="w-4 h-4 text-[#4f46e5] shrink-0 mt-0.5" strokeWidth={1.5} /> Approche personnalisée et adaptée</li>
                <li className="flex gap-3"><Leaf className="w-4 h-4 text-[#4f46e5] shrink-0 mt-0.5" strokeWidth={1.5} /> Cadre sécurisant et bienveillant</li>
              </ul>
            </div>
