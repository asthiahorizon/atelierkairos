      ...
      <main className="min-h-screen bg-[#f5f4f8] flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="glass-strong rounded-3xl p-10 w-full max-w-md">
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-2xl glass-indigo flex items-center justify-center"><Lock className="w-4 h-4 text-[#4338ca]" /></div>
          ...
          <form onSubmit={login} className="space-y-4">
            ...
            <Button type="submit" className="w-full bg-[#312e81] hover:bg-[#4338ca] text-white rounded-full py-5">Se connecter</Button>
          </form>
        </div>
    ...
    <main className="min-h-screen bg-[#f5f4f8] relative">
      ...
      <header className="sticky top-0 z-40 glass-strong">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <button onClick={logout} className="inline-flex items-center gap-2 text-sm text-[#312e81]/70 hover:text-[#312e81] transition-colors">
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="flex flex-wrap gap-2 mb-8">
          {TYPES.map((t) => (
            <button key={t.key} onClick={() => { setActiveType(t.key); setEditing(null); }} className={`px-5 py-2.5 rounded-full text-sm transition-all ${activeType === t.key ? 'bg-[#312e81] text-white' : 'glass text-[#312e81] hover:glass-strong'}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {editing ? (
          <div className="glass-strong rounded-3xl p-8 mb-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-serif text-2xl text-[#312e81]">{editing === 'new' ? 'Nouvelle entrée' : 'Modifier'}</h2>
              <button onClick={cancel} className="text-[#312e81]/60 hover:text-[#312e81]"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={save} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <Button type="submit" className="bg-[#312e81] hover:bg-[#4338ca] text-white rounded-full px-6"><Save className="w-4 h-4 mr-2" /> Enregistrer</Button>
                <Button type="button" variant="outline" onClick={cancel} className="rounded-full">Annuler</Button>
              </div>
            </form>
          </div>
        ) : (
          <button onClick={startNew} className="mb-6 inline-flex items-center gap-2 px-5 py-3 bg-[#312e81] text-white rounded-full text-sm hover:bg-[#4338ca] transition-colors"><Plus className="w-4 h-4" /> Nouvelle entrée</button>
        )}

        <div className="glass rounded-3xl overflow-hidden">
          <div className="divide-y divide-[#312e81]/10">
            {loading && <div className="p-8 text-center text-[#312e81]/60">Chargement…</div>}
            {!loading && entries.length === 0 && <div className="p-8 text-center text-[#312e81]/60">Aucune entrée pour le moment.</div>}
            {entries.map((e) => (
              <div key={e.id} className="p-5 flex items-start gap-4 hover:bg-white/30 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {e.description && <p className="text-sm text-[#312e81]/65 mt-1 line-clamp-2">{e.description}</p>}
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => togglePublish(e)} className="p-2 rounded-lg hover:bg-white/50 text-[#312e81]/70" title={e.published ? 'Dépublier' : 'Publier'}>{e.published ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}</button>
                  <button onClick={() => startEdit(e)} className="p-2 rounded-lg hover:bg-white/50 text-[#312e81]/70"><Pencil className="w-4 h-4" /></button>
