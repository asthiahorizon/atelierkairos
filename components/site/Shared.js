      ...
      <div className="container mx-auto px-6 relative">
        {kicker && (
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-indigo mb-7">
            ...
            <span className="text-[11px] tracking-[0.25em] uppercase text-[#3730a3]">{kicker}</span>
          </div>
        )}
        <h1 className="font-serif text-5xl md:text-6xl lg:text-7xl xl:text-[88px] leading-[1.02] tracking-tight max-w-5xl text-balance">
          {title} {italic && <span className="italic font-light text-[#4f46e5]">{italic}</span>}
        </h1>
        {subtitle && (
          <p className="mt-8 max-w-2xl text-lg md:text-xl text-[#3730a3]/70 leading-relaxed">{subtitle}</p>
        )}
      </div>
    </section>
  );
      ...
      <div className="container mx-auto px-6">
        <div className="relative overflow-hidden glass-dark text-white rounded-[2.5rem] p-10 md:p-16 lg:p-20">
          ...
          <div className="relative max-w-3xl">
            ...
            <Link href={href} className="mt-10 inline-flex items-center gap-2 px-7 py-4 bg-white text-[#3730a3] rounded-full text-sm hover:bg-[#a5b4fc] transition-all group">
              {buttonLabel}
    ...
    <div className={`max-w-3xl ${center ? 'mx-auto text-center' : ''} mb-12 md:mb-16`}>
      {kicker && (
        <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full glass-indigo mb-5 ${center ? 'mx-auto' : ''}`}>
          <span className="text-[10px] tracking-[0.25em] uppercase text-[#3730a3]">{kicker}</span>
        </div>
      )}
      <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl leading-[1.02] tracking-tight text-balance">
        {title} {italic && <span className="italic font-light text-[#4f46e5]">{italic}</span>}
      </h2>
      {subtitle && <p className="mt-6 text-lg text-[#3730a3]/70 leading-relaxed">{subtitle}</p>}
    </div>
  );
}
...
export function TagList({ tags, label = 'Indications possibles' }) {
  return (
    <div>
      {label && <p className="text-xs uppercase tracking-widest text-[#3730a3]/55 mb-4">{label}</p>}
      <div className="flex flex-wrap gap-2">
        {tags.map((tag) => (
          <span key={tag} className="text-xs md:text-sm px-3.5 py-1.5 rounded-full glass-indigo text-[#3730a3]">{tag}</span>
        ))}
      </div>
    </div>
  );
